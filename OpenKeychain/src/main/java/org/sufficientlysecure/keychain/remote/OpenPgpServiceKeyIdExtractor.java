...
class OpenPgpServiceKeyIdExtractor {
    ...
    KeyIdResult returnKeyIdsFromIntent(Intent data, boolean askIfNoUserIdsProvided,
        ...
        if (hasKeysFromSelectPubkeyActivity) {
            ...
            result = returnKeyIdsFromEmails(data, userIds, trustIdentityDao);
        } else {
    ...
    private KeyIdResult returnKeyIdsFromEmails(Intent data, String[] encryptionUserIds,
        ...
        if (hasUserIds) {
            Uri queryUri = EmailStatus.CONTENT_URI.buildUpon().appendPath(
                    trustIdentityDao.getCallingPackageName()).build();
            Cursor cursor = contentResolver.query(
                    queryUri, PROJECTION_KEY_SEARCH, null, encryptionUserIds, null);
            if (cursor == null) {
